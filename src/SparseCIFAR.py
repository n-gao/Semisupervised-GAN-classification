...
class SparseCIFAR(datasets.CIFAR10):
    def __init__(self, root, keep_percent, train=True, transform=None, target_transform=None, download=False, ign=-100,
        ...
        """
        super(SparseCIFAR, self).__init__(root, train, transform, target_transform, download)

        self.showAll = showAll
        N_samples = super(SparseCIFAR, self).__len__()
        if self.showAll:
    ...
    def __getitem__(self, index):
        if self.showAll:
            img, target = super(SparseCIFAR, self).__getitem__(index)
            if not self.filter[index]:
                target = self.ign
        else:
            img, target = super(SparseCIFAR, self).__getitem__(self.choice[index])
